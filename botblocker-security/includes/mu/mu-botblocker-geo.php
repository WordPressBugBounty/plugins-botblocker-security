 ...
 */
function bbcs_geo_valid_code( $value ): string {
	if ( ! is_string( $value ) ) {
		return '';
	}
 ...
 */
function bbcs_geo_prefix_ok( bool $is_v6, $prefix ): bool {
	$prefix = (int) $prefix;
	if ( $is_v6 ) {
		return $prefix >= 40;
	}
 ...
 */
function bbcs_geo_mmdb_path( $data_dir ): string {
 ...
 */
function bbcs_geo_header_country(): string {
	...
	foreach ( $keys as $key ) {
		if ( isset( $_SERVER[ $key ] ) && is_string( $_SERVER[ $key ] ) ) {
			...
			$code = bbcs_geo_valid_code( $_SERVER[ $key ] );
			if ( $code !== '' ) {
				return $code;
			}
		}
 ...
 */
function bbcs_geo_mmdb_lookup( $autoloader, $mmdb_path, string $ip ): ?array {
	static $cache = array();
	...
	if ( is_string( $autoloader ) && $autoloader !== '' && is_string( $mmdb_path ) && $mmdb_path !== '' && is_file( $mmdb_path ) ) {
		...
		if ( $size !== false && $size >= BBCS_ASN_DB_MIN_SIZE && $size <= BBCS_ASN_DB_MAX_SIZE ) {
			if ( bbcs_include_autoloader( $autoloader, 'BotBlocker\\Vendor\\MaxMind\\Db\\Reader' ) ) {
				try {
					...
					$country = ( is_array( $record ) && isset( $record['country'] ) )
						? bbcs_geo_valid_code( $record['country'] )
						: '';
					if ( $country !== '' ) {
						$result = array(
							'country' => $country,
 ...
 */
function bbcs_geo_load_blocked( $geo_file ): array {
	$blocked = array();
	if ( ! is_string( $geo_file ) || $geo_file === '' || ! file_exists( $geo_file ) ) {
		return $blocked;
	}
	if ( ! function_exists( 'bbcs_safe_load_data_file' ) ) {
		return $blocked;
	}
	$data = bbcs_safe_load_data_file( $geo_file );
	...
	foreach ( $data['bbcs_geo'] as $entry ) {
		...
		$code = bbcs_geo_valid_code( isset( $entry['code'] ) ? $entry['code'] : '' );
		if ( $code !== '' ) {
			$blocked[ $code ] = true;
		}
	}
 ...
 */
function bbcs_geo_is_allowed( array $rules, string $ip ): bool {
	if ( ! isset( $rules[ $ip ] ) ) {
		return false;
	}
 ...
 * @return bool
 */
function bbcs_geo_block_decision( string $header_country, $mmdb, array $blocked, bool $is_allowed_ip, bool $is_v6 ): bool {
	if ( $is_allowed_ip ) {
		return false;
	}
	if ( empty( $blocked ) ) {
	...
	if ( ! bbcs_geo_prefix_ok( $is_v6, $mmdb['prefix'] ?? 0 ) ) {
		return false;
	}
