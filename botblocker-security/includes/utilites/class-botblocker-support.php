 ...
 */
class BotBlockerSupport {

	public static function register(): void {
		add_action( 'bbcs_show_support_button', array( self::class, 'render_support_button' ) );
		add_action( 'wp_ajax_bbcs_send_support', array( self::class, 'handle_support_request' ) );
		add_action( 'admin_init', array( self::class, 'maybeSendActivationToCloud' ) );
	}

	public static function render_support_button(): void {
		?>
	...
	public static function handle_support_request(): void {
		...
		if ( ! BotBlockerIp::rateLimit( 'bbcs_send_support', 10, HOUR_IN_SECONDS ) ) {
	...
	public static function maybeSendActivationToCloud(): void {
		if ( get_option( 'bbcs_support_data' ) ) {
			return;
		}
		update_option( 'bbcs_support_data', 1, true );
		BotBlockerInstall::sendActivationToCloud();
	}
}
