...
class BotBlockerUI {
    ...
    protected static function load_market(): array {
        $market = [];
        $marketUrl = function_exists('bbcs_get_addons_url') ? bbcs_get_addons_url() : (defined('BOTBLOCKER_ADDONS') ? BOTBLOCKER_ADDONS : '');
        if ( $marketUrl ) {
    ...
    public static function get_addons_context(): array {
        ...
        foreach ( $addons as $addon ) {
            if ( $addon['update_avail'] ) { $updates_count++; }
        }
    ...
    public static function render_dashboard_addons_summary(): void {
        ...
        if ( empty( $addons ) ) {
            $addons_page = ($BBCSA && isset($BBCSA->pages_addons)) ? $BBCSA->pages_addons : bbcs_admin_page_url('bbcs_addons');
