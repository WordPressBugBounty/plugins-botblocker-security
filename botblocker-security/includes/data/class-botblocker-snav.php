...
class BotBlockerSnav {

	/** @var array|null */
	private static $core_groups = null;
	 ...
	 * @return array
	 */
	public static function getGlobalSearchIndex(): array {
		if ( self::$core_groups !== null ) {
		...
		$core_groups = array(
			...
			array(
				...
				'tabs' => array(
					...
					array(
						't'   => __( 'In-App Browser Mode', 'botblocker-security' ),
						'tab' => 'in-app',
						'go'  => 'settings',
						'sg'  => array(
							array(
								't' => __( 'In-App Browser Mode', 'botblocker-security' ),
								's' => array(
									array( __( 'Enable In-App Browser Mode', 'botblocker-security' ), 'bbcs_inapp_enabled' ),
									array( __( 'Rescue groups', 'botblocker-security' ), 'bbcs_inapp_rescue_codes' ),
								),
							),
						),
					),
				...
				'tabs' => array(
					array(
						...
						'sg'  => array(
							...
							array(
								't' => __( 'Audit Log', 'botblocker-security' ),
								's' => array(
									array( __( 'Enable Audit Log', 'botblocker-security' ), 'audit_log_enable' ),
									array( __( 'Audit Log Retention', 'botblocker-security' ), 'audit_log_retention_days' ),
									array( __( 'Roles to Audit', 'botblocker-security' ), 'audit_log_roles' ),
								),
							),
						),
					),
	 ...
	 *
	 * Built once from getGlobalSearchIndex() and statically cached.
	 * All routing functions (getSettingLink, getHealthKeyTabMap,
	 * findTabByHealthKey) derive their data from this single source.	 *
	 * @return array<string, array{page: string, tab: string, label: string}>
	 */
	public static function getSettingIndex(): array {
		if ( self::$index !== null ) {
			return self::$index;
		}

		$index = array();
		$tree  = self::getGlobalSearchIndex();
	 ...
	 * Auto-derived from getGlobalSearchIndex() via getSettingIndex().
	 * Used by findTabByHealthKey() and status-page links.
	 *
	 * @return array<string, string> health_key => tab_id
	 */
	public static function getHealthKeyTabMap(): array {
		if ( self::$map !== null ) {
			return self::$map;
		}

		$map = array();
		foreach ( self::getSettingIndex() as $key => $route ) {
			$map[ $key ] = $route['tab'];
		}
	 ...
	 */
	public static function findTabByHealthKey( string $health_key ): ?string {
		$map = self::getHealthKeyTabMap();
	 ...
	 */
	public static function getTabpanels(): array {
	 ...
	 * @return string[]
	 */
	public static function getSimpleTabIds(): array {
		return array(
			'simple-detection',
			'connection-types',
			'browser-plugins',
	 ...
	 *
	 * Uses the centralized getSettingIndex() for O(1) lookup instead of
	 ...
	 */
	public static function getSettingLink( string $key, bool $focus = false ): string {
		$index = self::getSettingIndex();
