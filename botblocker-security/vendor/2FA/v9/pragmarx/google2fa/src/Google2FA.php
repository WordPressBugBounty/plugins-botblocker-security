...
namespace BotBlocker\Vendor\PragmaRX\Google2FA;

use BotBlocker\Vendor\PragmaRX\Google2FA\Exceptions\InvalidAlgorithmException;
use BotBlocker\Vendor\PragmaRX\Google2FA\Exceptions\SecretKeyTooShortException;
use BotBlocker\Vendor\PragmaRX\Google2FA\Support\Base32;
use BotBlocker\Vendor\PragmaRX\Google2FA\Support\Constants;
use BotBlocker\Vendor\PragmaRX\Google2FA\Support\QRCode;
class Google2FA
{
    use QRCode;
    use Base32;
    ...
    )
    {
        for (; $startingTimestamp <= $timestamp + $this->getWindow($window); $startingTimestamp++) {
            if (hash_equals($this->oathTotp($secret, $startingTimestamp), (string) $key)) {
                return is_null($oldTimestamp) ? \true : $startingTimestamp;
            }
        }
        return \false;
    }
    ...
    {
        return (int) floor(microtime(\true) / $this->keyRegeneration);
    }
    /**
    ...
    {
        return [Constants::SHA1, Constants::SHA256, Constants::SHA512];
    }
    /**
     * Get the OTP window.
     *
    ...
    )
    {
        ...
        return str_pad($this->oathTruncate($this->generateHotp($secret, $counter)), $this->getOneTimePasswordLength(), '0', \STR_PAD_LEFT);
    }
    /**
    ...
    )
    {
        ...
        return substr((string) $temp, -$this->getOneTimePasswordLength());
    }
    /**
    ...
    )
    {
        ...
        return $this->findValidOTP($secret, $key, $window, $this->makeStartingTimestamp($window, $timestamp, $oldTimestamp), $timestamp, $oldTimestamp);
    }
    /**
     * Verifies a user inputted key against the current timestamp. Checks $window
