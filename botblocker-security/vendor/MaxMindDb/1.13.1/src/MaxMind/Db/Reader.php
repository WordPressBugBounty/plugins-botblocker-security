...
namespace BotBlocker\Vendor\MaxMind\Db;

use BotBlocker\Vendor\MaxMind\Db\Reader\Decoder;
use BotBlocker\Vendor\MaxMind\Db\Reader\InvalidDatabaseException;
use BotBlocker\Vendor\MaxMind\Db\Reader\Metadata;
use BotBlocker\Vendor\MaxMind\Db\Reader\Util;
/**
...
class Reader
{
    ...
    public function __construct(string $database)
    {
        ...
        if (is_dir($database)) {
            ...
            throw new InvalidDatabaseException("Error opening database file ({$database}). Is this a valid MaxMind DB file?");
        }
    ...
    private function findAddressInTree(string $ipAddress): array
    {
        ...
        if ($rawAddress === \false) {
            throw new InvalidDatabaseException('Could not unpack the unsigned char of the packed in_addr representation.');
        }
    ...
    private function readNode(int $nodeNumber, int $index): int
    {
        ...
        switch ($this->metadata->recordSize) {
            case 24:
                ...
                if ($rc === \false) {
                    throw new InvalidDatabaseException('Could not unpack the unsigned long of the node.');
                }
                [, $node] = $rc;
                return $node;
            case 28:
