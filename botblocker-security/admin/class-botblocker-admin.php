...
class Botblocker_Admin {
	...
	/**
	 * Screens migrated to the new admin UI.
	 *
	 * Each screen is added here only after its template is ported. While empty,
	 * no pages load the new bbcs-* CSS/JS and the legacy bundle is used unchanged.
	 */
	private function get_new_ui_screen_ids(): array {
		return array(
			'toplevel_page_bbcs_dashboard',
			'botblocker_page_bbcs_settings',
			'botblocker_page_bbcs_setup_guide',
	...
	private function use_new_admin_ui(): bool {
		return defined( 'BBCS_NEW_ADMIN_UI' ) && BBCS_NEW_ADMIN_UI;
	}

	/**
	 * Whether the new unified admin UI stylesheet/JS bundle should be enqueued.
	 *
	 * Delegates to use_new_admin_ui() - the two gates have been merged.
	 */

	private function render_new_page( string $page, ...$args ): void {
	...
	public function enqueue_styles(): void {
		...
		// New UI styles take over only on migrated screens when enabled.
		if ( $this->use_new_admin_ui() && in_array( $screen->id, $this->get_new_ui_screen_ids(), true ) ) {
			$this->enqueue_new_ui_styles( $screen );
			return;
		}

		// Legacy styles are loaded per page so a migrated page can drop them.
		$this->enqueue_legacy_styles( $screen );
	}

	/**
	 * New unified admin UI stylesheet bundle.
	 *
	 * Loaded only on migrated screens when BBCS_NEW_ADMIN_UI is enabled.
	...
	/**
	 * Legacy stylesheet bundle, dispatched per admin screen.
	 *
	 * Old CSS is no longer enqueued from a single global block; each screen
	 * pulls in exactly what it needs so a migrated screen can skip it entirely
	 * when BBCS_NEW_ADMIN_UI is enabled.
	 */
	private function enqueue_legacy_styles( WP_Screen $screen ): void {
		$this->enqueue_legacy_base_styles();
	...
	/**
	 * Shared legacy stylesheet bundle used by every non-migrated screen.
	 */
	private function enqueue_legacy_base_styles(): void {
		$deps = array( 'common' );
	...
	public function enqueue_scripts(): void {
		...
		// New UI scripts take over only on migrated screens when enabled.
		if ( $this->use_new_admin_ui() && in_array( $screen->id, $this->get_new_ui_screen_ids(), true ) ) {
			$this->enqueue_new_ui_scripts();
			return;
		}

		$this->enqueue_legacy_scripts( $screen );
	}
	...
	private function enqueue_new_ui_scripts(): void {
		...
		if ( $this->is_screen( $screen->id, 'toplevel_page_bbcs_dashboard' ) ) {
			...
			wp_enqueue_script( BOTBLOCKER_SHORT_NAME . '-dash-js', plugin_dir_url( __FILE__ ) . 'js/bbcs-js/bbcs-dashboard.js', array( 'jquery', BOTBLOCKER_SHORT_NAME . '-multipage-js' ), BOTBLOCKER_VERSION, true );
		...
		wp_enqueue_script( BOTBLOCKER_SHORT_NAME . '-shared-helpers-js', plugin_dir_url( __FILE__ ) . 'js/bbcs-js/bbcs-shared-helpers.js', array( 'jquery' ), BOTBLOCKER_VERSION, true );
		wp_localize_script(
		...
		if ( $this->is_screen( $screen->id, 'botblocker_page_bbcs_settings' ) || $this->is_screen( $screen->id, 'botblocker_page_bbcs_integrations' ) || $this->is_screen( $screen->id, 'botblocker_page_bbcs_tools' ) || $this->is_screen( $screen->id, 'botblocker_page_bbcs_addons' ) ) {
			...
			if ( function_exists( 'bbcs_get_global_search_index' ) ) {
				wp_add_inline_script(
					...
					'var BOTBLOCKER_GLOBAL_SEARCH_INDEX = ' . wp_json_encode( bbcs_get_global_search_index() ) . ';',
					'before'
				);
			}
		}
		...
		// GROUPS are now auto-derived from bbcs_get_global_search_index() inside bbcs_get_palette_data().
		if ( function_exists( 'bbcs_get_palette_data' ) ) {
			$palette = bbcs_get_palette_data();
			wp_add_inline_script(
				BOTBLOCKER_SHORT_NAME . '-multipage-js',
	...
	/**
	 * Legacy script bundle, dispatched per admin screen.
	 *
	 * Old JS is no longer enqueued from a single global block; each screen
	 * pulls in exactly what it needs so a migrated screen can skip it entirely
	 * when BBCS_NEW_ADMIN_UI is enabled.
	 */
	private function enqueue_legacy_scripts( WP_Screen $screen ): void {
		global $pagenow;
	...
	public function dashboard_page(): void {
		...
		if ( $this->use_new_admin_ui() ) {
			...
			$this->render_new_page( 'dashboard', $view, $layout );
		} else {
	...
	public function settings_page(): void {
		if ( $this->use_new_admin_ui() ) {
			...
			$this->render_new_page( 'settings', $view, $layout );
		} else {
	...
	public function reports_page(): void {
		if ( $this->use_new_admin_ui() ) {
			...
			$this->render_new_page( 'reports', $view, $layout );
		} else {
	...
	public function rules_page(): void {
		if ( $this->use_new_admin_ui() ) {
			...
			$this->render_new_page( 'rules', $view, $layout );
		} else {
	...
	public function tools_page(): void {
		if ( $this->use_new_admin_ui() ) {
			...
			$this->render_new_page( 'tools', $view, $layout );
		} else {
	...
	public function integrations_page(): void {
		if ( $this->use_new_admin_ui() ) {
			...
			if ( empty( $bbcs_secret ) && isset( $bbcs_google_auth ) ) {
			...
			if ( empty( $bbcs_backup_codes ) && function_exists( 'bbcs_generate_backup_codes' ) ) {
			...
			$this->render_new_page( 'integrations', $view, $layout );
		} else {
	...
	public function cloud_api_page(): void {
		if ( $this->use_new_admin_ui() ) {
			...
			$this->render_new_page( 'cloud-api', $view, $layout );
		} else {
	...
	public function addons_page(): void {
		if ( $this->use_new_admin_ui() ) {
			...
			$this->render_new_page( 'addons', $view, $layout );
		} else {
	...
	public function setup_guide_page(): void {
		if ( $this->use_new_admin_ui() ) {
			...
			$layout = new Botblocker_Layout_View( $vm, true );
			$this->render_new_page( 'setup-guide', $view, $layout );
		} else {
	...
	public function about_page(): void {
		if ( $this->use_new_admin_ui() ) {
			...
			$this->render_new_page( 'about', $view, $layout );
		} else {
