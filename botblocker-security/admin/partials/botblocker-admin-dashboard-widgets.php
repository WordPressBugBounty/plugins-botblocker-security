...
function bbcs_dashboard_assets(): void {
	...
	if ($screen && $screen->id === 'dashboard') {
		...
		wp_localize_script(BOTBLOCKER_SHORT_NAME . '-dashboard', 'botblockerData', array(
...
function bbcs_register_custom_dashboard_widgets(): void
{
...
add_action('wp_dashboard_setup', 'bbcs_register_custom_dashboard_widgets');

function bbcs_display_news_widget(): void
{
...
function bbcs_display_stats_widget(): void
{
	$BBCSA = Botblocker_Admin::getInstance();
	bbcs_collect_statistic_data();
	$has_pro = bbcs_isCloudAPIActive();
	?>
		<div style="position: absolute; z-index: 100;">
			<div class="bbcs-card-actions">
				...
				<a href="<?php echo esc_url($BBCSA->pages_settings); ?>" class="bbcs-icon-button" data-bs-toggle="tooltip" data-bs-html="true" data-bs-placement="top"
					data-bs-original-title="<?php echo esc_attr_e('Settings', 'botblocker-security'); ?>">
		...
		<div class="bbcs-health-wrapper"> 
			<div class="col-lg-6">
				<?php echo do_shortcode('[bbcs_health_gauge id="health_gauge" value="' . bbcs_calculateSiteHealth() .
					'" max="100"]'); ?>
...
function bbcs_display_form_widget(): void
{ 
	?>
		...
		<form id="addIpRuleForm" class="bbcs-qr-form" novalidate>
			<div class="bbcs-qr-grid">
				...
				<div class="bbcs-qr-field">
					...
					<input type="datetime-local" class="bbcs-qr-input" id="addExpires" name="expires" value="<?php echo esc_attr(wp_date('Y-m-d\TH:i', strtotime('+1 day'), wp_timezone())); ?>" required>
