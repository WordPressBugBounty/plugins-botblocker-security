...
return static function ( Botblocker_DashboardViewModel $data ): void {
	...
	$link = static function ( string $url, string $label, string $desc ): string {
		return '<div class="bbcs-col bbcs-g-0h">'
			. '<div class="bbcs-fw-semibold bbcs-fs-sm">' . esc_html( $label ) . '</div>'
			. '<div class="bbcs-row bbcs-g-1h">'
			. '<input type="text" class="bbcs-input bbcs-input--mono bbcs-fill" value="' . esc_url( $url ) . '" readonly onclick="this.select()" />'
		...
		<div class="bbcs-modal">
			...
			<div class="bbcs-modal-body">
				<div class="bbcs-col bbcs-g-3">
					...
					<?php echo $link( $data->secret->disable_url, __( 'Disable current check', 'botblocker-security' ), __( 'Temporarily disable BotBlocker checks for this session.', 'botblocker-security' ) ); ?>
					<?php echo $link( $data->secret->off_url, __( 'Fully disable BotBlocker', 'botblocker-security' ), __( 'Completely disable BotBlocker protection.', 'botblocker-security' ) ); ?>
					<?php echo $link( $data->secret->on_url, __( 'Re-enable BotBlocker', 'botblocker-security' ), __( 'Re-enable BotBlocker protection.', 'botblocker-security' ) ); ?>
