			...
			<div>
				...
				SettingsGroup::make()
					...
					->withItems( static function () use ( $data ): void {
						...
						ToggleOption::make()->withName( 'block_override' )->withChecked( $data->is_checked( 'block_override' ) )->withLabel( __( 'Override Detection', 'botblocker-security' ) )->withTooltip( __( 'Detect tampered browser properties (navigator, plugins).', 'botblocker-security' ) )->withBadge( 'PRO', ToggleOption::BADGE_PRO )->withDisabled( ! $data->has_pro )->render();
						ToggleOption::make()->withName( 'block_web_engine_options' )->withChecked( $data->is_checked( 'block_web_engine_options' ) )->withLabel( __( 'Engine Parameter Checks', 'botblocker-security' ) )->withTooltip( __( 'Verify browser engine parameters against known browser signatures.', 'botblocker-security' ) )->withBadge( 'PRO', ToggleOption::BADGE_PRO )->withDisabled( ! $data->has_pro )->render();
						ToggleOption::make()->withName( 'block_device_options' )->withChecked( $data->is_checked( 'block_device_options' ) )->withLabel( __( 'Device API Verification', 'botblocker-security' ) )->withTooltip( __( 'Verify device APIs (touch, motion sensors) to detect headless browsers.', 'botblocker-security' ) )->withBadge( 'PRO', ToggleOption::BADGE_PRO )->withDisabled( ! $data->has_pro )->render();
